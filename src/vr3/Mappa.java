...
import java.io.*;
import java.net.*;            // per il supporto client-server
import java.util.ArrayList;
import java.util.List;
import java.util.Map;
import java.util.HashMap;
import java.util.Collections;  // per MapServer
...
public class Mappa extends JFrame {
    ...
    // AGGIUNTA: Istanza del server per il client-server
    private static MapServer server = null;

    // AGGIUNTA: contatore aggiornamenti per garantire l'ordine (inizialmente 0)
    private static int updateSeq = 0;
    ...
    public Mappa() {
        ...
        // AGGIUNTA: Avvio del server per il client-server (porta 5000)
        try {
            server = new MapServer(5000);
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(this, "Errore nell'avvio del server: " + ex.getMessage());
        }

        setVisible(true);
    }

    // Metodo per aggiornare la finestra secondaria e inviare aggiornamenti al client-server
    private void updateSecondaryFrame(int r, int c) {
        ...
        // Invio aggiornamento al server con numero di sequenza
        if (server != null) {
            updateSeq++; // incremento per ogni aggiornamento
            String hex = String.format("#%02x%02x%02x",
                    grid[r][c].getBaseColor().getRed(),
                    grid[r][c].getBaseColor().getGreen(),
                    grid[r][c].getBaseColor().getBlue());
            String occupant = grid[r][c].getOccupantCharacter();
            if (occupant == null) occupant = "";
            String message = "UPDATE;" + updateSeq + ";" + r + ";" + c + ";" + hex + ";" + occupant;
            server.broadcast(message);
        }
    }

    // ---------------------------
    // Metodo per modificare la dimensione delle celle
    // ---------------------------
...
// ---------------------------
// Classe MapServer per il supporto client-server
// ---------------------------
class MapServer {
